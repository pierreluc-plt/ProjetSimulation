    ...
    if forme == 'triangle':
        ...
        while int(larg) > 1:
            if up == 1:
                ...
                Map[int(centre_bois_x - larg / 2):int(centre_bois_x + larg / 2) + 1,
                int(centre_bois_y - Ny_Bois / 2) - i] = 2
        ...
        if larg1 == 3:
            ...
            Map[int(centre_bois_x - (larg1 - 2) / 2):int(centre_bois_x + (larg1 - 2) / 2),
            int(centre_bois_y - Ny_Bois / 2) - i] = 4
        ...
        for x in x_val1:
            ...
            for i in np.arange(1, 5):
                if (Map[int(x), int(pos_y + i)] != 6 and Map[int(x - 1), int(pos_y + i)] == 1 and Map[
                    int(x - 1), int(pos_y)] == 1 \
...
def surfdemicercle(S_x, S_y, centre_bois_x, centre_cercle_y, Nx_Bois, Ny_Bois, centre_bois_y, coeff):
    limite_pente = (-(
                -2 * S_y * centre_bois_x + 2 * centre_bois_x * centre_cercle_y + 2 * S_x * (S_y - centre_cercle_y)) + (
                        np.sqrt((-2 * S_y * centre_bois_x + 2 * centre_bois_x * centre_cercle_y + 2 * S_x * (
                                    S_y - centre_cercle_y)) ** 2 - (4 * (
                                    2 * S_x * centre_bois_x + coeff ** 2 - centre_bois_x ** 2 - S_x ** 2) * (
                                                                                coeff ** 2 - (
                                                                                    S_y - centre_cercle_y) ** 2))))) / (
    ...
    surf = 2 * coeff * np.arcsin(
        (np.sqrt((limite_x - centre_bois_x - Nx_Bois / 2) ** 2 + (centre_bois_y - Ny_Bois / 2 - limite_y) ** 2)) / (
                    2 * coeff))
    return surf
...
def surface_directe(S_x, S_y, centre_bois_x, centre_bois_y, Nx_Bois, Ny_Bois, forme, coeff):
    if S_x < (centre_bois_x - Nx_Bois / 2):
        if forme == 'triangle':
            ...
            y_max = centre_bois_y - Ny_Bois / 2 - Nx_Bois / (2 * np.tan(coeff / 2)) - (centre_bois_x - S_x) / np.tan(
                coeff / 2)
            if S_y <= y_max:
            ...
            if S_y > centre_bois_y - Ny_Bois / 2:
                ...
                if S_pente > coin_pente:
                    # calcul arc
                    surf = surfdemicercle(S_x, S_y, centre_bois_x, centre_cercle_y, Nx_Bois, Ny_Bois, centre_bois_y,
                                          coeff)
        ...
        if forme == 'triangle':
            if S_x < centre_bois_x:
                y_max = centre_bois_y - Ny_Bois / 2 - Nx_Bois / (2 * np.tan(coeff / 2)) - (
                            centre_bois_x - S_x) / np.tan(coeff / 2)
                if S_y < y_max:
                ...
                y_max = centre_bois_y - Ny_Bois / 2 - Nx_Bois / (2 * np.tan(coeff / 2)) - (
                            S_x - centre_bois_x) / np.tan(coeff / 2)
                if S_y < y_max:
                    surface = 2 * (Nx_Bois / 2) / (np.sin(coeff / 2))
                elif S_y < (centre_bois_y - Ny_Bois / 2) and S_y >= y_max:
                    surface = (Nx_Bois / 2) / (np.sin(coeff / 2))
            ...
            if S_y > (centre_bois_y + Ny_Bois / 2):
                ...
                surface = surfdemicercle(S_x, S_y, centre_bois_x, centre_cercle_y, Nx_Bois, Ny_Bois, centre_bois_y,
                                         coeff)
            elif S_x == centre_bois_x:
                surface = 2 * coeff * np.arcsin(Nx_Bois / (2 * coeff))
            elif S_x > centre_bois_x:
                surface = 2 * coeff * np.arcsin(Nx_Bois / (2 * coeff)) - surfdemicercle(S_x, S_y, centre_bois_x,
                                                                                        centre_cercle_y, Nx_Bois,
                                                                                        Ny_Bois, centre_bois_y, coeff)
    elif S_x > (centre_bois_x + Nx_Bois / 2):
        if forme == 'triangle':
            y_min = centre_bois_y - Ny_Bois / 2 + (S_x - centre_bois_x - Nx_Bois / 2) / np.tan(coeff / 2)
            y_max = centre_bois_y - Ny_Bois / 2 - Nx_Bois / (2 * np.tan(coeff / 2)) - (S_x - centre_bois_x) / np.tan(
                coeff / 2)
            if S_y <= y_max:
                surf = 2 * (Nx_Bois / 2) / (np.sin(coeff / 2))
            elif S_y <= y_min and S_y > y_max:
            ...
            if S_y > centre_bois_y - Ny_Bois / 2:
                ...
                if S_pente < coin_pente:
                    # calcul arc
                    surf = 2 * coeff * np.arcsin(Nx_Bois / (2 * coeff)) - surfdemicercle(S_x, S_y, centre_bois_x,
                                                                                         centre_cercle_y, Nx_Bois,
                                                                                         Ny_Bois, centre_bois_y, coeff)
                elif S_pente >= coin_pente and S_y <= (centre_bois_y + Ny_Bois / 2):
                    surf = 0
                ...
                surf = 2 * coeff * np.arcsin(Nx_Bois / (2 * coeff)) - surfdemicercle(S_x, S_y, centre_bois_x,
                                                                                     centre_cercle_y, Nx_Bois, Ny_Bois,
                                                                                     centre_bois_y, coeff)
            elif S_y < centre_cercle_y - coeff:
                S_pente = -(centre_bois_y - Ny_Bois / 2 - S_y) / (S_x - centre_bois_x + Nx_Bois / 2)
                if S_pente < coin_pente:
...
def Coeff_PML(Type, i, j, h, Nx, Ny, k2_eau, v_eau, N_PML):
    ...
    if np.logical_or(np.logical_or(i == 0, i == Ny - 1), np.logical_or(j == 0, j == Nx - 1)):
        ...
        if Type == 14:
            ...
            Coeff = [0, Beta_x / 2 * h / (Gamma_x ** 2) + 1 / (Gamma_x ** 2), 0,
                     Beta_y / 2 * h / (Gamma_y ** 2) + 1 / (Gamma_y ** 2) \
                , -2 / ((Gamma_y) ** 2) - 2 / ((Gamma_x) ** 2) + k ** 2 * h ** 2,
                     -Beta_x / 2 * h / (Gamma_x ** 2) + 1 / (Gamma_x ** 2), 0,
                     -Beta_y / 2 * h / (Gamma_y ** 2) + 1 / (Gamma_y ** 2), 0]
...
def Construction_Map(Nx, Ny, Nx_Bois, Ny_Bois, centre_bois_x, centre_bois_y, forme, coeff, S_x, S_y, dx, N_PML,
                     plot=True, \
...
def Plots_Results(MapSol, MapSolSB, MapSol_TFSF, Display_Map, Interpolation="none"):
    ...
    ax[0][0].imshow(np.transpose(np.log10(np.real(MapSol) - 1.2 * np.min(np.real(MapSol)))), cmap="jet", alpha=1,
    ...
    ax[0][1].imshow(np.transpose(np.log10(np.real(MapSolSB) - 1.2 * np.min(np.real(MapSolSB)))), alpha=1.0, cmap="jet",
                    interpolation=Interpolation)
...
def SER3(Sx, Sy, SF_only, Source_Map, p_source, centre_bois_x, centre_bois_y, R_integration, dx, forme, coeff, Nx_Bois,
         Ny_Bois):
    # R_integration est en mètre

    if forme == "triangle":
        Distance_bateau_bois = (np.sqrt((Sx - centre_bois_x) ** 2 + (Sy - centre_bois_y) ** 2) - (
                    Ny_Bois / 2 + Nx_Bois / 2 / np.tan(coeff / 2))) * dx
