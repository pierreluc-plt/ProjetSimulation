...
S=zeros(Ny,Nx); B=zeros(Ny,Nx); alpha=zeros(Ny,Nx); rho=zeros(Ny,Nx);
for i=1:Ny
    y=(i-1)*d;
    for j=1:Nx
        x=(j-1)*d;
        
%                 if (x>=1.9)&&(x<=1.9+d)&&(y>=0.9)&&(y<=0.9+d)
%                     % À l'intérieur de l'élément chauffant
%                     S(i,j)=exp(-1i.*((x-1.8)+(y-1.2).^2));
%                 else
%                     % À l'extérieur de l'élément chauffant
%                     S(i,j)=0;
%                 end
        
        S(i,j)=exp(-1i*(((x-1.8).^2) + ((y-1.2).^2)));
        
        % Délimitation des espaces
        if (x>=2)&&(x<=2.5)&&(y>=1)&&(y<=1.5)
            % Bois
            B(i,j) = 640.72;
            alpha(i,j) = alpha_b;
            rho(i,j) = rho_b;
